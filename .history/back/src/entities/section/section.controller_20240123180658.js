...
const addSections = (req, res) => {
    ...
    pool.query(queries.getSectionByKeyword, [keyword], (error, resultsKeyword) => {
        console.log(keyword);
        console.log(resultsKeyword.rows.length);
        console.log(resultsKeyword);
        if (resultsKeyword.rows.length) {
            return res.status(404).json({ message: 'Keyword ya emleada' });
        }

        pool.query(
            queries.addSectionsQuery,
            [title, description, blogid, keyword, structure],
            (error, results) => {
                if (error) throw error;
