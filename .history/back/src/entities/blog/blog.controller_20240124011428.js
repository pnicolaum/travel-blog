...
// filtros
const getBlogsByContinent = (req, res) => {
    const { continent } = req.params;
    pool.query(queries.getBlogsByContinentQuery, [continent], (error, results) => {
        if (error) throw error;
        if (results.rows.length === 0) {
            return res.status(404).json({ message: 'Blog no encontrado' });
        }
...
const getBlogsByCountry = (req, res) => {
    ...
    pool.query(queries.getBlogsByCountryQuery, [country], (error, results) => {
        if (error) throw error;
        if (results.rows.length === 0) {
...
const getBlogsByDays = (req, res) => {
    const days = parseInt(req.params.days);
    pool.query(queries.getBlogsByDaysQuery, [days], (error, results) => {
        if (error) throw error;
        if (results.rows.length === 0) {
            return res.status(404).json({ message: 'Blog no encontrado' });
        }
...
const getFilter = (req, res) => {
    if (req.query.length > 0) {
        const { continent, country, days } = req.query;
    } else {
        const { continent, country, days } = req.body;
    }
